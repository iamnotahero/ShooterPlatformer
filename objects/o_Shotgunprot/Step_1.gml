...
if (key_unequip) {
	isequip = 0};
if (mouse_check_button_pressed(mb_left)) and (firingdelay < 0 ) and cur_ammo > 0 and reloading = 0 {
	cur_ammo-=1;
	firingdelay = 50;
	recoil = 10;
	if o_Player.iscrouched == false{
	playerknockback = recoil - 3}else{
		playerknockback = recoil - 8}
	for (var i = 0; i < 5; i += 1){
	with (instance_create_layer(x,y,"Bullet",o_BulletShotgun)){
		speed = 25;
...
//show_debug_message(o_Player.y - lengthdir_y(playerknockback,image_angle))
with(o_Player){
if (!place_meeting(x+5,y,o_Wall))and (!place_meeting(x-5,y,o_Wall)){
x = x  - lengthdir_x(other.playerknockback,other.image_angle)}
if (!place_meeting(x,y+4,o_Wall)) and (!place_meeting(x,y-5,o_Wall)){
